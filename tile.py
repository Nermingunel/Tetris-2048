...
class Tile:
    ...
    def __init__(self, position=Point(0, 0)):  # (0, 0) is the default position
        ...
        if n == 1:
            ...
            self.background_color = Color(255, 209, 128)  # background (tile) color
            self.foreground_color = Color(255, 243, 224)  # foreground (number) color
            self.boundary_color = Color(232, 245, 233)  # boundary (box) color
            # set the position of the tile as the given position
        elif n == 2:
            # set the colors of the tile
            self.background_color = Color(255, 171, 64)  # background (tile) color
            self.foreground_color = Color(255, 243, 224)  # foreground (number) color
            self.boundary_color = Color(232, 245, 233)  # boundary (box) color
        elif n == 3:
            # set the colors of the tile
            self.background_color = Color(255, 145, 0)  # background (tile) color
            self.foreground_color = Color(255, 243, 224)  # foreground (number) color
            self.boundary_color = Color(232, 245, 233)  # boundary (box) color
