...
class GameGrid:
    ...
    def __init__(self, grid_h, grid_w):
        ...
        self.empty_cell_color = Color(128, 203, 196)
        # set the colors used for the grid lines and the grid boundaries
        self.line_color = Color(232, 245, 233)
        self.boundary_color = Color(232, 245, 233)
    ...
    def draw_score(self):
        font_family, font_size = "Arial", 21
        boundary_thickness = 0.004
        score_color = Color(255, 255, 255)
        stddraw.setPenColor(score_color)
        stddraw.setPenRadius(boundary_thickness)
        stddraw.setFontFamily(font_family)
        stddraw.setFontSize(font_size)
        stddraw.boldText(self.grid_width * 0.06, self.grid_height * 0.95, "Score: " + str(self.score))
        stddraw.setPenRadius()  # reset the pen radius to its default value
    ...
    def check_full_filled_rows(self):
        ...
        while row < n_rows:
            ...
            if column_count == n_cols:
                ...
                self.score += 1
                self.move_all_tiles_down(row)
            else:
                row += 1

    def move_all_tiles_down(self, given_row):
        n_rows, n_cols = len(self.tile_matrix), len(self.tile_matrix[0])
        for row in range(given_row + 1, n_rows):
            for col in range(n_cols):
                if self.tile_matrix[row][col] != None:
