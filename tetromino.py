...
class Tetromino:
    ...
    # Method for rotating the tetromino in 90 degree counter clockwise
    def rotate(self, direction, game_grid):
        if not (self.can_be_moved(direction, game_grid)):
            return False  # tetromino cannot be moved in the given direction
        ...
        for x in range(0, int(N / 2)):
            ...
            for y in range(x, N - x - 1):
                ...
                if self.tile_matrix[y][N - 1 - x] != None:
                    y_dir = y - x
                    x_dir = y - (N - 1 - x)
                    self.tile_matrix[x][y].move(x_dir, y_dir)

                # move values from bottom to right
                self.tile_matrix[y][N - 1 - x] = self.tile_matrix[N - 1 - x][N - 1 - y]
                if self.tile_matrix[N - 1 - x][N - 1 - y] != None:
                    y_dir = (N - 1 - x) - y
                    x_dir = N - 1 - x - (N - 1 - y)
                    self.tile_matrix[y][N - 1 - x].move(x_dir, y_dir)

                # move values from left to bottom
                self.tile_matrix[N - 1 - x][N - 1 - y] = self.tile_matrix[N - 1 - y][x]
                if self.tile_matrix[N - 1 - y][x] != None:
                    y_dir = (N - 1 - y) - (N - 1 - x)
                    x_dir = N - 1 - y - x
                    self.tile_matrix[N - 1 - x][N - 1 - y].move(x_dir, y_dir)
                ...
                if temp != None:
                    y_dir = x - (N - 1 - y)
                    x_dir = x - y
                    self.tile_matrix[N - 1 - y][x].move(x_dir, y_dir)
    ...
    def can_be_moved(self, dir, game_grid):
        n = len(self.tile_matrix)  # n = number of rows = number of columns
        if dir == "left" or dir == "right" or dir == "space":
            for row in range(n):
                for col in range(n):
                    ...
                    # direction = right --> check the rightmost tile of each row
                        ...
                        if self.tile_matrix[row][col] != None:
                            ...
                            if game_grid.is_occupied(leftmost.y, leftmost.x - 1):
                                return False
                            break  # end the inner for loop
                        if self.tile_matrix[row][n - 1 - col] != None:
                            rightmost = self.tile_matrix[row][n - 1 - col].get_position()
                            # tetromino cannot go right if any of its rightmost tiles is
                            # at x = grid_width - 1
                            if rightmost.x == self.grid_width - 1:
